    ...
    <div className="mx-auto flex min-h-[80vh] max-w-3xl flex-col justify-center px-4 py-8">
      <div className="card p-6 sm:p-8">
        <div className="mb-6 text-center">
          <div className="inline-flex h-16 w-16 items-center justify-center rounded-full bg-orange-100 text-3xl">
            🛵
          </div>
          <h1 className="mt-3 text-2xl font-bold">Únete como repartidor</h1>
          ...
          {STEPS.map((s, i) => (
            <div key={s.n} className="flex flex-1 items-center">
              <div className="flex flex-col items-center gap-1">
                ...
                <span
                  className={cn(
                    ...
                    step === s.n ? "text-brand-600 font-semibold" : "text-neutral-400",
                  )}
                >
                  {s.title}
                </span>
              ...
              {i < STEPS.length - 1 && (
                <div className={cn(
                  "mx-1 h-0.5 flex-1",
                  step > s.n ? "bg-green-500" : "bg-neutral-200",
                )} />
              )}
            </div>
          ))}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    {GENDERS.map((g) => (
                      <label key={g.value} className={cn(
                        "flex flex-1 cursor-pointer items-center justify-center rounded-lg border-2 px-3 py-2 text-sm font-medium",
                        form.gender === g.value
          ...
          {step === 3 && (
            <Section title="Vehículo" icon={<Car className="h-4 w-4" />}>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {VEHICLES.map((v) => (
                    <label key={v.value} className={cn(
                      "flex cursor-pointer flex-col items-center gap-1 rounded-lg border-2 p-3 transition",
              ...
              >
                <input className="input-base uppercase" placeholder="ABC-123"
                       value={form.vehicle_plate ?? ""}
                       onChange={(e) => update("vehicle_plate", e.target.value)} />
              </Field>

              <div className="rounded-lg bg-neutral-50 p-4">
                <p className="mb-3 text-xs font-semibold uppercase text-neutral-600">
        ...
        <p className="text-center text-xs text-neutral-400">
          ¿Eres cliente?{" "}
          <Link to="/register" className="hover:underline">Regístrate aquí</Link>
        </p>
      </div>
    </div>
  );
...
function Section({
  ...
  return (
    <div className="space-y-3">
      <h3 className="flex items-center gap-2 text-sm font-bold uppercase tracking-wide text-neutral-600">
